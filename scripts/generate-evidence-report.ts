...
const layerRules: LayerRule[] = [
  ...
  { name: 'Layer 1: Core Mathematical Engines', level: 1, pattern: /utils\/(calculations|statusMatrixEngine|enterpriseUpgradeEngine)\.ts$/, allowedImports: [0, 1] },
...
function run() {
  ...
  console.log(`${colors.bright}${colors.cyan}  DOCUSIGHT PROGRAMMATIC EVIDENCE REPORT & GRAPH EXTRACTOR  ${colors.reset}`);
  console.log(`${colors.bright}${colors.cyan}================================================================================${colors.reset}\n`);
  ...
  const markdownContent = `# DocuSight Mathematical & Architecture Evidence Log
*Generated on ${new Date().toISOString()} via DocuSight Custom AST Compliance Engine*
